...
function checked(f) {
  ...
  var $name = $(f).find('input[name="fname"]');
  var $phone = $(f).find('input[name="fphone"]');

  $.each([$name, $phone], function(i, v) {
    if (v.val() == "") {
      v.addClass("err");
      v.parent()
        .find("span.err")
...
function ajaxAdd(el, product_id) {
  ...
  var selector = el.parents(".products__item").find(".product-option-wrap");
...
function addCartSelf(t) {
  var sendData = [];

  $("#options_" + t)
    .find("input.quantity_st")
    .each(function() {
      if ($(this).val() != 0) {
        sendData.push({
          tovar: t,
  ...
  $.ajax({
    url: "/bitrix/templates/zaobkz2017/addToCart.php",
    global: false,
    type: "POST",
